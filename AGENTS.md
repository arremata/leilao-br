...
- **2026-07-14** — Módulo jurídico real (fim do mock): `AuctionPropertyResult` ganhou campo `legal` (`LegalDetail` em `contracts.py`, espelhando `legalDemo.js`); `legal.py` reescrito seguindo o guia Anthropic de sumarização jurídica — prompt pt-BR ramificado por modalidade (CPC / Lei 9.514 / venda direta), guardrails anti-alucinação com estados verificado/não-localizado/requer-humano e citação literal do edital, meta-sumarização para editais > 24k chars (fim do truncamento em 8k), validação Pydantic com retry e degradação para `risk_level="unknown"`; modelo configurável via `LEGAL_MODEL`; custo de desocupação extraído do edital (fallback R$ 10k). Remoção total do Tavily (código morto, testes, docs). Frontend: fix crash do GlossaryDrawer, glossário interativo linkificado na aba Jurídica (termos → drawer filtrado), fix hero da Home com R$ 0, convenção única de sinal do desconto (`DiscountLabel` em shared.jsx: positivo sem sinal, negativo `+X% acima/ágio`), estados de loading/erro na carga inicial, contraste de `--fg-3` para AA, cabeçalho da Watchlist alinhado às colunas novas, nav/cards acessíveis por teclado, coluna ROI do Histórico → avaliação, marca "Arremate" residual → Argos. `AuctionState.errors` com reducer `operator.add` (market+legal em paralelo estouravam InvalidUpdateError). Proxy do Vite: `127.0.0.1:8000` (IPv6 fix no Windows).
