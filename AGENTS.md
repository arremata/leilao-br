...
- **2026-09-24** — Added the buyer landing page at `/morar`, written for people buying their first home who have never been to an auction. It is a self-contained static page (`frontend/public/morar.html`, CSS and JS inline so the service worker's cache-first static strategy cannot serve a stale copy) with a Vercel rewrite from `/morar` and `/morar/` placed ahead of the SPA fallback. The page follows the buyer communication plan: total cost to the key instead of the bid, occupancy and payment badges, debts described per auction type without generalizing, a video-demo slot, and an example property that reproduces the app's property page (tabs 01–04, cost table with weights). Its simulator starts from how much the buyer has available and shows the highest offer that fits once every cost is counted. Every call to action leads to the catalog; the page makes no API calls and has no write path.
