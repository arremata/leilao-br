...
- **2026-09-16** — Added a complete local account prototype with create-account and sign-in modes, required name/e-mail/password/confirmation/terms fields, password visibility control, local session and salted password verifier, followed by the four-step housing questionnaire. The account screen now occupies the viewport without the application navigation or desktop scrolling, keeping every registration field visible in a compact split layout. No server authentication or database was added; the adapter boundary is documented for that future integration.
